...
router.post("/listAddFood", (req, res) => {
  const {food} = req.body;
  console.log("in list add food")
  console.log(food)
  ...
  List.findOneAndUpdate({username: "user1"}, {$push: {listfoods: food}}, err =>{
    if (err) return res.json({ success: false, error: err });
    return res.json({ success: true });
  })
});

router.get("/listGetGroceryList", (req, res) => {
  List.find((err, data) => {
    if (err) return res.json({ success: false, error: err });
    return res.json({ success: true, data: data });
  });
